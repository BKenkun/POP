...
export default function CheckoutClientPage() {
  ...
  const handleAddressSelection = (
    addressId: string,
    currentAddresses: Address[] = userDoc?.addresses || []
  ) => {
  ...
  const handleApplyCoupon = useCallback(async () => {
    ...
    try {
      ...
      if (!coupon.isActive)
        throw new Error(t('checkout.toasts.coupon_error_inactive'));
      if (coupon.startDate && now < new Date(coupon.startDate))
        throw new Error(t('checkout.toasts.coupon_error_not_yet_valid'));
      if (coupon.endDate && now > new Date(coupon.endDate))
        throw new Error(t('checkout.toasts.coupon_error_expired'));
      if (
        coupon.usageLimit &&
        coupon.usageCount >= coupon.usageLimit
      )
        throw new Error(t('checkout.toasts.coupon_error_limit_reached'));
      if (coupon.minPurchase && cartTotal < coupon.minPurchase)
        throw new Error(
          t('checkout.toasts.coupon_error_min_purchase', {
            price: formatPrice(coupon.minPurchase),
          })
        );
      ...
      toast({
        title: t('checkout.toasts.coupon_applied_title'),
        description: t('checkout.toasts.coupon_applied_desc', {
          discount: formatPrice(discount),
        }),
      });
    } catch (error: any) {
      toast({
        title: t('checkout.toasts.coupon_error_title'),
    ...
    try {
        ...
        const localOrderData: Omit<Order, 'id'> = {
            ...
            items: cartItems.map(item => ({
                productId: item.id, name: item.name, price: item.price, quantity: item.quantity, imageUrl: item.imageUrl,
            })),
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl md:text-4xl font-headline text-primary mb-8 text-center font-bold">
        {t('checkout.title')}
      </h1>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onFinalSubmit)}>
          {step === 1 && (
            <Card>
              <CardHeader>
                <CardTitle>{t('checkout.confirm_cart_title')}</CardTitle>
              </CardHeader>
              <CardContent>
                {cartItems.map((item) => (
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-muted-foreground">
                  {user
                    ? t('checkout.user_details_subtitle')
                    : t('checkout.user_details_not_logged_in')}
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                {!user ? (
                  <Alert variant="destructive">
                    ...
                    <AlertTitle>
                      {t('checkout.login_required_alert_title')}
                    </AlertTitle>
                    <AlertDescription>
                  ...
                  <>
                    {addresses.length > 0 && (
                      <RadioGroup
                        ...
                        onValueChange={(id) =>
                          handleAddressSelection(id, addresses)
                        }
                      ...
                      >
                        {addresses.map((addr, index) => (
                          <Label
                            ...
                            htmlFor={addr.id}
                            className={cn(
                              'flex flex-col rounded-lg border p-4 cursor-pointer hover:bg-primary/10 transition-colors',
                        ...
                        <Label
                          htmlFor="new"
                          className={cn(
                            'flex items-center justify-center gap-2 rounded-lg border-2 border-dashed p-4 cursor-pointer hover:bg-primary/10 transition-colors',
                    ...
                    >
                      <h3 className="font-bold text-lg mb-4">
                        {t('checkout.shipping_address_title')}
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('checkout.city_label')}</FormLabel>
                              <FormControl>
                                <Input {...field} />
                              </FormControl>
                              <FormMessage />
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('checkout.state_label')}</FormLabel>
                              <FormControl>
                                <Input {...field} />
                              </FormControl>
                              <FormMessage />
          ...
          {step === 3 && (
            <Card>
              <CardHeader>
                <CardTitle>{t('checkout.review_title')}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        {couponLoading ? (
                          ...
                          t('checkout.applied_button')
                        ) : (
                          t('checkout.apply_button')
                        )}
                      </Button>
                    </div>
          ...
          <div className="mt-8 flex justify-between items-center">
            {step > 1 ? (
              <Button type="button" variant="outline" onClick={handlePrevStep}>
                <ArrowLeft className="mr-2" /> {t('checkout.previous_button')}
              </Button>
            ) : (
              <Button asChild type="button" variant="outline">
                <Link href="/products">
                  &larr; {t('checkout.continue_shopping')}
                </Link>
              </Button>
            )}
            {step < 3 && (
